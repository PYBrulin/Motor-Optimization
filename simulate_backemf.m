...
%% Select range of currents to simulate
...
while j <= length(Current)
    ...
    %% Check at which iteration the simulation stopped previously
    if (startiterat == 0)

        while ((j < length(Current)) ...
                && (exist(fullfile(filepath, sprintf('results/%s/result_%08.3f.csv', g.name, Current(j))), 'file')) ...
                && (exist(fullfile(filepath, sprintf('results/%s/result_%08.3f.csv', g.name, Current(j + 1))), 'file')))
            fprintf('[%s] Found result file %s\n', datestr(now, 0), fullfile(filepath, sprintf('results/%s/result_%08.3f.csv', g.name, Current(j))));
            j = j + 1;
        end

        result_file = sprintf('results/%s/result_%08.3f.csv', g.name, Current(j));
    ...
    %% Safeguard layer
    % Wine (on Linux) crashes a lot of times during the execution of this script.
    ...
    state_file = sprintf('results/%s/state.txt', g.name);

    if ~exist('retry_times', 'var')
    ...
    try
        init_geometry(g, InitialAngle - StepAngle * startiterat, id, Current(j), 0);
    catch
    ...
    for i = startiterat:niterat
        ...
        if i == 0
            ...
            % See README on why we need to redraw the geometry if ratio < 1
            if ratio < 1
                closefemm;
        ...
        try
            ...
            fprintf('[%s] Caught Exception - Analyze\n', datestr(now, 0))
            % Try to restart femm
            closefemm;
            openfemm;
            retry_times = retry_times + 1;
        ...
        try
            ...
            fprintf('[%s] Caught Exception - Loading model\n', datestr(now, 0))
            % Try to restart femm
            closefemm;
            openfemm;
            retry_times = retry_times + 1;
